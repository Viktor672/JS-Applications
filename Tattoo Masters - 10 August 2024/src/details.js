...
const template = (data, totalLikes, hasUserLiked, likeHandler) => {
  let hasUserLoggeedIn = localStorage.getItem('accessToken') !== null;
  ...
  return html`
   <section id="details">
      ...
      ''
                  ...
                  ${!isOwner && !hasUserLiked && hasUserLoggeedIn
      ?
...
async function getTotalLikes(id) {
  let url = `${baseUrl}/data/likes?where=tattooId%3D%22${id}%22&distinct=_ownerId&count`;
  return await fetchData(url);
}

async function checkIfUserLiked(id, userId) {
  let url = `${baseUrl}/data/likes?where=tattooId%3D%22${id}%22%20and%20_ownerId%3D%22${userId}%22&count`;
  return await fetchData(url);
}
